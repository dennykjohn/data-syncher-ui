...
const scheduleFromBatch = (batch: MigrationBatch): ScheduleValue => ({
...
const EditBatchModal = ({
  ...
  if (open && (formSource.open !== open || formSource.batchId !== batch.id)) {
    setFormSource({ open, batchId: batch.id });
    setName(batch.name);
    setSchedule(scheduleFromBatch(batch));
  }

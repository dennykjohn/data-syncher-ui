...
const batchesKey = (connectionId: number) => ["batches", connectionId] as const;
...
export function useFetchBatches(connectionId: number, enabled: boolean = true) {
  return useQuery<FetchBatchesResponse>({
    queryKey: batchesKey(connectionId),
...
export function useCreateBatch(connectionId: number) {
  ...
  return useMutation({
    ...
    mutationFn: (payload: CreateBatchPayload) =>
      AxiosInstance.post<MigrationBatch>(
        ServerRoutes.connector.batches.list(connectionId),
        payload,
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: batchesKey(connectionId) });
    },
  });
}
